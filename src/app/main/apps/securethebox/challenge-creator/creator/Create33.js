...
mutation Create33Mutation(
    ...
    $slug: String!){
      createCourse(
        clusterData:{ status: $clusterStatus, name: $clusterName},
        categoryData: {value:$categoryValue,label:$categoryLabel,color: $categoryColor},
        courseData: { title: $title, description: $description, activeStep: $activeStep, length: $length, totalSteps: $totalSteps, slug: $slug }){
      course{
        title
        description
        activeStep
...
function createCourse(environment, variables) {
    commitMutation(
        environment,
        {
            mutation,
...
class Create3 extends Component {
    ...
    render() {
        const onSubmit = async values => {
            console.log(values)
            { createCourse(environment, values) }
        }
